...
import { CalendarEvent, SearchResult } from '../types';
...
export async function initializeQdrantCollection(): Promise<boolean> {
  try {
...
export async function indexCalendarEvent(event: CalendarEvent): Promise<boolean> {
  try {
...
export async function deleteCalendarEventVector(eventId: string): Promise<boolean> {
  try {
...
export async function searchSimilarEvents(query: string, userId: string, limit: number = 5): Promise<SearchResult[]> {
  try {
    ...
    return searchResult.map((result) => ({
      ...result.payload as CalendarEvent,
      score: result.score,
...
// Indexer en masse des événements
export async function batchIndexEvents(events: CalendarEvent[]): Promise<boolean> {
  try {
    const points = await Promise.all(events.map(async (event) => {
      const text = `${event.title} ${event.description || ''}`;
      const embedding = await getEmbedding(text);
    ...
    console.log(`${points.length} événements indexés avec succès`);
    return true;
  } catch (error) {
    console.error('Erreur lors de l\'indexation en masse des événements:', error);
    return false;
  }
} 